...
RESOURCE_NAME = "ACA BUSINESS GROUP - Documente Contractuale"


def ensure_contract_resource(db) -> CatalogResource:
    existing = db.execute(
...
def main() -> None:
    source_files = sorted(
        [
            *CONTRACTS_DIR.glob("aca_business_group_*.md"),
            *CONTRACTS_DIR.glob("aca_business_group_*.docx"),
        ]
    )
    if not source_files:
        raise SystemExit("Nu exista documente ACA BUSINESS GROUP in docs/contracts.")

    db = SessionLocal()
    try:
        resource = ensure_contract_resource(db)
        created = 0
        skipped = 0
